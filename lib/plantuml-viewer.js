...
module.exports.openViewerForEditor = function (editor) {
    var uri = createPlantumlViewerUri(editor)
    var viewer_pane = atom.workspace.paneForURI(uri)
    var grammars = atom.config.get('plantuml-viewer.grammars') || []
    if (grammars.indexOf(editor.getGrammar().scopeName) === -1) return
    if (!viewer_pane) addViewerForUri(uri)
    else {
        var editor_view = viewer_pane.itemForURI(uri)
        if (!editor_view) addViewerForUri(uri)
        else {
            // View already exists -> bring it to front..
            var prevActivePane = atom.workspace.getActivePane()
            viewer_pane.activateItem(editor_view)
            // ..and put focus back to the editor.
